...
- `bot/client.py` (`SystemClient`): async HTTP client with persistent session, retries/backoff and consistent error messages. All requests include `Authorization: Bearer <AUTH_TOKEN>`.
...
- `client/server.py`: Flask app. Endpoints: `/` (health), `/status`, `/command` (switch on `command`), `/upload`, `/getfile`, `/download/<filename>`.
  - Commands implemented: `lock`, `volume`, `mute`, `copy`, `paste`, `screenshot` (Linux), `sleep`, `shutdown`, `battery_status`, `network_info`, `network_stats`, plus process/media commands if added.
...
- First run requires system packages: Linux `scrot`, `alsa-utils`, `xclip`/`xsel`; install Python deps from each `requirements.txt`.
- Dev loop: start Flask client first, then bot. Bot error messages surface client unavailability (“Python client not running...” from `SystemClient`).

## Implementation patterns

- Auth: always call `authorize(message)` early; return silently for non-owner.
- Exclusivity: wrap potentially long or spammed actions (volume, screenshot) with `CommandManager.run_exclusive(...)` to auto-cancel previous task.
- Contract: POST `/command` with JSON `{"command": "<name>", "params": {...}}`; expect JSON `{'status': 'success'|'error', 'message': str, ...}`.
- OS branching lives only in Flask handlers (`client/handlers/*`); bot layer stays purely async I/O.
- UX: Use Reply Keyboard labels verbatim (e.g. `'🔒 Lock Screen'`); avoid inline keyboard unless extending legacy path.
- Safe edits/deletes via `utils.safe_edit` / `utils.safe_delete` to swallow Telegram API edit races.

## Guardrails

- AUTH mismatch → 401 from Flask; handle gracefully at bot with user-facing error.
- Screenshots require valid `DISPLAY` and `scrot`; default `:0` is injected if missing.
- Download restrictions: `/getfile` denies paths outside allowed dirs; handle user file path errors with clear message.
- Legacy inline handlers reference `process_*` / `media_*` commands—implement them server-side before wiring or they will return “Unknown command”.

## Adding a new command (example)

Client (`client/server.py`): add mapping entry: `COMMAND_MAP['reboot'] = lambda p: {'status': 'success', 'message': '🔁 Rebooting...'} (plus OS call).`

Bot (`bot/bot.py`): add button `'🔁 Reboot'`, register handler with `authorize`, wrap in `run_exclusive`, call `client.send_command('reboot')`, then edit using `result_icon`.

## When in doubt

- Mirror patterns in `bot/bot.py` and modular handlers under `client/handlers/`.
- Keep network + system logic server-side; never run OS commands from the bot.
- Favor small, composable handler functions returning the standard JSON format.
